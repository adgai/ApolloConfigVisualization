...
class CodemanSettingsConfigurable : Configurable {
    ...
    private val settings = CodemanGlobalSettings.instance
    ...
    override fun apply() {
        ...
        configuration.state.keyValues.clear()
        currentKeyValues.forEach { keyValue ->
            configuration.addKeyValue(keyValue.key, keyValue.value)
        }

        configuration.state.cookie = cookieTextField?.text ?: ""
        configuration.state.foldingWhenEveryOpenFile = foldingWhenEveryOpenFileRadioButton!!.isSelected
        configuration.state.methodSignatures = methodSignatureTextField?.text ?: ""
        configuration.state.account = accountTextField?.text ?: ""
        configuration.state.password = String(passwordField?.password ?: CharArray(0))
    }

    override fun reset() {
        tableModel!!.rowCount = 0 // 清空表格
        val configuration = settings.state
